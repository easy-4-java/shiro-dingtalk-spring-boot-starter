...
class ShiroDingTalkWebAutoConfigurationTest {
    ...
    @Test
    @DisplayName("Auto-configuration loads when 'shiro.dingtalk.enabled=true'")
    void testLoadsWhenEnabledPropertySet() {
        runner.withUserConfiguration(ShiroDingTalkWebAutoConfiguration.class)
                .withPropertyValues("shiro.dingtalk.enabled=true")
                .run(context -> assertThat(context).hasSingleBean(ShiroDingTalkWebAutoConfiguration.class));
    }
    ...
    void testNotLoadedWhenPropertyAbsent() {
        runner.withUserConfiguration(ShiroDingTalkWebAutoConfiguration.class)
                .run(context -> assertThat(context).doesNotHaveBean(ShiroDingTalkWebAutoConfiguration.class));
    }
}
